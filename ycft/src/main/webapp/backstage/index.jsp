...
<head>
   ...
   <%List<Privilege> pList = (List)request.getAttribute("pList"); %>
</head>
<body>
    <div id="wrapper">
        ...
        <nav class="navbar-default navbar-side" role="navigation">
            <div class="sidebar-collapse">
               <ul class="nav" id="main-menu">
				...
				%>
					<li>
                        <a href="#"><%=pList.get(i).getIcon() %> <%=pList.get(i).getmName() %><span class="fa arrow"></span></a>
                        <ul class="nav nav-second-level">
                            <%for(int j = 0;j<pList.get(i).getChildMenus().size();j++){ %>
                            <li>
                                <a href="<%=pList.get(i).getChildMenus().get(j).getmUrl()%>"><%=pList.get(i).getChildMenus().get(j).getmName() %></a>
                            </li>
                            <%} %>
                        </ul>
                    </li>
   ...
   $(".nav-second-level a").click(function(){//点击的时候给当前这个加上，其他的移除
	  $(this).addClass("active-menu").parent().siblings().find("a").removeClass("active-menu"); 
   });
   </script>
